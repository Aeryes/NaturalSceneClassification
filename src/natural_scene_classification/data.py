...
def enumerate_labeled_items(root: Path) -> list[DatasetItem]:
    ...
    for label, class_name in enumerate(CLASS_NAMES):
        for file_path in _iter_class_files(root, class_name):
...
def load_data(dataset_path: str | Path, transform=None) -> SceneDataset:  # type: ignore[no-untyped-def]
    return SceneDataset(enumerate_labeled_items(Path(dataset_path)), transform=transform)
...
def create_split_manifest(
    ...
    *,
    ...
    for label, class_name in enumerate(CLASS_NAMES):
        files = _iter_class_files(train_root, class_name)
        shuffled = files[:]
    ...
    rows.sort(
        key=lambda row: (str(row["split"]), str(row["class_name"]), str(row["relative_path"]))
    )
...
def read_split_manifest(
    manifest_path: Path, train_root: Path = DEFAULT_TRAIN_DIR
...
def build_dataset_manifest(
    *,
    ...
    train_summary = inspect_dataset(data_root / "seg_train" / "seg_train")
    test_summary = inspect_dataset(data_root / "seg_test" / "seg_test")
    payload = {
        "manifest_version": 1,
...
def download_kaggle_dataset(
    *,
    ...
    archive_path = data_root / "intel-image-classification.zip"
    ...
    if manifest_path is None:
        manifest_path = Path("configs") / "intel-scenes.dataset.json"
...
def validate_expected_layout(data_root: Path = DEFAULT_DATA_ROOT) -> None:
    inspect_dataset(data_root / "seg_train" / "seg_train")
    inspect_dataset(data_root / "seg_test" / "seg_test")
