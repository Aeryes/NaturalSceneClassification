...
from natural_scene_classification.config import load_toml_file
from natural_scene_classification.constants import (
    DEFAULT_DATA_ROOT,
    DEFAULT_PREDICT_DIR,
    DEFAULT_SEED,
)
...
from natural_scene_classification.evaluation import evaluate_checkpoint
from natural_scene_classification.inference import predict_folder, predict_image
from natural_scene_classification.training import TrainConfig, train_model
...
def _apply_train_config_overrides(args: argparse.Namespace) -> TrainConfig:
    ...
    return TrainConfig(
        ...
        epochs=cast(int, config_values.get("epochs", 3)),
        batch_size=cast(int, config_values.get("batch_size", 64)),
        ...
        learning_rate_head=cast(float, config_values.get("learning_rate_head", 5e-4)),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="nsc")
    ...
    train_parser.add_argument(
        "--no-pretrained-weights", action="store_false", dest="load_pretrained_weights"
    )

    evaluate_parser = subparsers.add_parser("evaluate")
    evaluate_parser.add_argument("--checkpoint", type=Path, required=True)
    evaluate_parser.add_argument(
        "--test-root", type=Path, default=DEFAULT_DATA_ROOT / "seg_test" / "seg_test"
    )
...
def main() -> None:
    ...
    if args.command == "data" and args.data_command == "download":
        ...
        print(
            json.dumps(
                {"archive_path": str(archive_path), "manifest_path": str(manifest_path)}, indent=2
            )
        )
        return
    ...
    if args.command == "data" and args.data_command == "inspect":
        ...
        manifest_path = args.manifest_path or Path("configs") / "intel-scenes.dataset.json"
        build_dataset_manifest(
            data_root=data_root,
            seed=args.seed,
        ...
        payload = {
            "train": inspect_dataset(data_root / "seg_train" / "seg_train"),
            "test": inspect_dataset(data_root / "seg_test" / "seg_test"),
