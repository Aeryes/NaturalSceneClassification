...
from natural_scene_classification.checkpoints import save_checkpoint, save_checkpoint_metadata
from natural_scene_classification.constants import CLASS_NAMES, DEFAULT_BATCH_SIZE, DEFAULT_EPOCHS
from natural_scene_classification.data import (
    SceneDataset,
    ensure_split_manifest,
    read_split_manifest,
)
from natural_scene_classification.modeling import ModelConfig, build_model
from natural_scene_classification.transforms import build_eval_transform, build_train_transform
...
def _compute_class_weights(dataset: SceneDataset, device: torch.device) -> torch.Tensor:
...
def train_model(config: TrainConfig) -> dict[str, object]:
    ...
    manifest_path = ensure_split_manifest(
        train_root=config.data_root / "seg_train" / "seg_train",
        manifest_path=config.split_manifest,
        seed=config.seed,
        validation_fraction=config.validation_fraction,
    )
    split_items = read_split_manifest(manifest_path, config.data_root / "seg_train" / "seg_train")

    train_dataset = SceneDataset(
        split_items["train"], transform=build_train_transform(config.image_size)
    )
    ...
    optimizer = torch.optim.Adam(
        [
            {"params": model.resnet.layer2.parameters(), "lr": config.learning_rate_backbone},
            {"params": model.resnet.layer3.parameters(), "lr": config.learning_rate_backbone},
